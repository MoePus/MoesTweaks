package com.moepus.moestweaks;

public class Config {
    public boolean hideShield = false;
    public boolean stopFire = false;
    public boolean villageSpawnPoint = false;
    public boolean stopNetherPortal = false;
    public boolean piglinTweaks = false;
    public boolean piglinWearNetherite = false;
    public boolean hoglinTweaks = false;
    public boolean moreSlime = false;
    public boolean mobThrowsMustDrop = false;
    public boolean bakaSilverFish = false;
    public boolean noPhantomSpawner = false;
    public boolean noPatrolSpawner = false;
    public boolean noCatSpawner = false;
    public boolean noVillageSiege = false;
    public boolean noWanderingTraderSpawner = false;
    public boolean betterKeepInv = false;
    public boolean doubleJump = false;
    public boolean adrenalineEffect = false;
    public boolean noRecipeBook = false;
    public boolean itemEntityRenderOptimization = true;
}